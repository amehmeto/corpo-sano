...
exports.buildQueryPlanningContext = exports.buildOperationContext = exports.collectSubfields = exports.findDirectivesOnNode = exports.buildQueryPlan = void 0;
...
const getOperationAndFragments_1 = require("./utilities/getOperationAndFragments");
...
function buildQueryPlan(operationContext, options = { autoFragmentization: false }) {
    ...
    const rootType = (0, graphql_1.getOperationRootType)(context.schema, context.operation);
...
function findDirectivesOnNode(node, directiveName) {
    var _a, _b;
    return ((_b = (_a = node.directives) === null || _a === void 0 ? void 0 : _a.filter((directive) => directive.name.value === directiveName)) !== null && _b !== void 0 ? _b : []);
}
exports.findDirectivesOnNode = findDirectivesOnNode;
function collectInclusionConditions(document) {
    const { operation, fragmentMap } = (0, getOperationAndFragments_1.getOperationAndFragments)(document);
    const conditionalTree = buildConditionalTree(operation.selectionSet, fragmentMap);
    ...
    if (inclusionConditions.some((v) => v.include === true && (v.skip === null || v.skip === false)) ||
        inclusionConditions.some((v) => v.skip === false && (v.include === null || v.include === true))) {
        return null;
    }
    return inclusionConditions;
}
function collectConditionalValues(selection) {
...
function extractConditionalValue(conditionalDirective) {
    const conditionalArg = conditionalDirective.arguments[0].value;
    return 'name' in conditionalArg
        ? conditionalArg.name.value
        : conditionalArg.value;
}
function buildConditionalTree(selectionSet, fragmentMap) {
    ...
    for (const selection of selectionSet.selections) {
        if (selection.kind === graphql_1.Kind.FIELD) {
            ...
            const { selectionSet } = selection.kind === graphql_1.Kind.FRAGMENT_SPREAD
                ? fragmentMap.get(selection.name.value)
                : selection;
            const { skip, include } = collectConditionalValues(selection);
            if (skip === null && include === null) {
                const nestedSelections = buildConditionalTree(selectionSet, fragmentMap);
                if (nestedSelections) {
...
function walkConditionalTree(tree, fn) {
    tree.forEach((node) => walkConditionalTreeNode(node, fn));
}
function walkConditionalTreeNode(node, fn) {
    if (Array.isArray(node)) {
        fn(node);
        node[1].forEach((n) => {
...
function executionNodeForGroup(context, { serviceName, fields, requiredFields, internalFragments, mergeAt, dependentGroups, }, parentType) {
    ...
    const requires = requiredFields.length > 0
        ? (0, FieldSet_1.selectionSetFromFieldSet)(requiredFields)
        : undefined;
    ...
    const fetchNode = {
        kind: 'Fetch',
        serviceName,
        requires: requires ? (0, QueryPlan_1.trimSelectionNodes)(requires === null || requires === void 0 ? void 0 : requires.selections) : undefined,
...
function splitSubfields(context, path, fields, parentGroup) {
    splitFields(context, path, fields, field => {
        ...
        if (!(0, graphql_1.isObjectType)(parentType) || ((_a = (0, composedSchema_1.getFederationMetadataForType)(parentType)) === null || _a === void 0 ? void 0 : _a.isValueType)) {
        ...
        if (owningService === baseService) {
            if (owningService === parentGroup.serviceName ||
                parentGroup.providedFields.some((0, FieldSet_1.matchesField)(field))) {
                return parentGroup;
            }
            else {
            ...
            if (requiredFields.every(requiredField => parentGroup.providedFields.some((0, FieldSet_1.matchesField)(requiredField)))) {
                if (owningService === parentGroup.serviceName) {
                    return parentGroup;
                }
                else {
...
function splitFields(context, path, fields, groupForField) {
    for (const fieldsForResponseName of (0, FieldSet_1.groupByResponseName)(fields).values()) {
        for (const fieldsForScope of (0, FieldSet_1.groupByScope)(fieldsForResponseName).values()) {
            debug.group(() => (0, FieldSet_1.debugPrintFields)(fieldsForScope));
            ...
            if ((0, graphql_1.isIntrospectionType)((0, graphql_1.getNamedType)(fieldDef.type))) {
                debug.groupEnd(`Skipping introspection type ${fieldDef.type}`);
                continue;
            }
            if ((0, graphql_1.isObjectType)(parentType) && scope.possibleRuntimeTypes().includes(parentType)) {
                ...
                const hasNoExtendingFieldDefs = !possibleFieldDefs.some((field) => { var _a; return (_a = (0, composedSchema_1.getFederationMetadataForField)(field)) === null || _a === void 0 ? void 0 : _a.graphName; });
...
function shouldIncludeToFetchGroup(group, fields, context, runtimeParentType) {
...
function completeField(context, scope, parentGroup, path, fields) {
    ...
    if (!(0, graphql_1.isCompositeType)(returnType)) {
        ...
        const fieldPath = addPath(path, (0, graphql_2.getResponseName)(fieldNode), fieldDef.type);
        ...
        debug.group(() => `Splitting collected sub-fields (${(0, FieldSet_1.debugPrintFields)(subfields)})`);
        splitSubfields(context, fieldPath, subfields, subGroup);
        debug.groupEnd();
        parentGroup.mergeDependentGroups(subGroup);
        let definition;
        let selectionSet = (0, FieldSet_1.selectionSetFromFieldSet)(subGroup.fields, returnType);
    ...
    mergeDependentGroups(that) {
        for (const dependentGroup of that.dependentGroups) {
            const existingDependentGroup = this.dependentGroups.find((group) => group.serviceName === dependentGroup.serviceName &&
                (0, deep_equal_1.default)(group.mergeAt, dependentGroup.mergeAt));
...
function debugPrintGroup(group) {
    let str = `Fetch(${group.serviceName}, ${(0, FieldSet_1.debugPrintFields)(group.fields)}`;
