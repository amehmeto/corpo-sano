...
exports.isASTKind = exports.getArgumentValuesForRepeatableDirective = exports.getArgumentValuesForDirective = exports.parseFieldSet = exports.astFromType = exports.allNodesAreOfSameKind = exports.getResponseName = exports.getFieldDef = void 0;
...
function parseFieldSet(source) {
    ...
    (0, assert_1.assert)(selections.length > 0, `Field sets may not be empty`);
...
function getArgumentValuesForDirective(directiveDef, node) {
    (0, assert_1.assert)(!directiveDef.isRepeatable, 'Use getArgumentValuesForRepeatableDirective for repeatable directives');
    if (!node.directives)
        return undefined;
    const directiveNode = node.directives.find((directiveNode) => directiveNode.name.value === directiveDef.name);
    if (!directiveNode)
        return undefined;
    return (0, values_1.getArgumentValues)(directiveDef, directiveNode);
}
...
function getArgumentValuesForRepeatableDirective(directiveDef, node) {
    ...
    return directiveNodes.map((directiveNode) => (0, values_1.getArgumentValues)(directiveDef, directiveNode));
}
