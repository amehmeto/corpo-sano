...
var InsertQueryBuilder = /** @class */ (function (_super) {
     ...
     */
    InsertQueryBuilder.prototype.orUpdate = function (statementOrOverwrite, conflictTarget, orUpdateOptions) {
    ...
    InsertQueryBuilder.prototype.createInsertExpression = function () {
        ...
        var returningExpression = (this.connection.driver instanceof OracleDriver_1.OracleDriver && this.getValueSets().length > 1)
            ? null
        ...
        if (this.connection.driver.supportedUpsertType === "on-conflict-do-update") {
            if (this.expressionMap.onIgnore) {
                ...
                if (Array.isArray(overwrite) && skipUpdateIfNoValuesChanged && this.connection.driver instanceof PostgresDriver_1.PostgresDriver) {
                    query += " WHERE (";
                    query += overwrite.map(function (column) { return tableName + "." + _this.escape(column) + " IS DISTINCT FROM EXCLUDED." + _this.escape(column); }).join(" OR ");
                    query += ") ";
                }
            }
        }
