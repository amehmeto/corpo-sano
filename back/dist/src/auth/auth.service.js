"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AuthService = void 0;
const common_1 = require("@nestjs/common");
const athlete_repository_interface_1 = require("../athlete/repositories/athlete-repository.interface");
const Bcrypt = require("bcrypt");
const jwt_1 = require("@nestjs/jwt");
const athlete_entity_1 = require("../athlete/entities/athlete.entity");
const uuid_1 = require("uuid");
const repository_errors_enum_1 = require("../athlete/types/repository-errors.enum");
const email_gateway_1 = require("./gateways/email.gateway");
const biometrics_entity_1 = require("../biometrics/entities/biometrics.entity");
let AuthService = class AuthService {
    constructor(jwtService, athleteRepository, emailGateway) {
        this.jwtService = jwtService;
        this.athleteRepository = athleteRepository;
        this.emailGateway = emailGateway;
    }
    async signIn(authCredentialsInput) {
        const { email, password } = authCredentialsInput;
        const athlete = await this.athleteRepository.findByEmail(email);
        if (!athlete || (await this.isWrongPassword(password, athlete.password)))
            throw new common_1.UnauthorizedException();
        const payload = { athleteId: athlete.id };
        const token = this.jwtService.sign(payload);
        return { token };
    }
    async isWrongPassword(password, athleteHashedPassword) {
        return !(await Bcrypt.compare(password, athleteHashedPassword));
    }
    async register(registerAthleteInput) {
        try {
            const { name, email, password, biometrics } = registerAthleteInput;
            const hashedPassword = await this.hash(password);
            const athlete = new athlete_entity_1.Athlete({
                id: (0, uuid_1.v4)(),
                biometrics: new biometrics_entity_1.Biometrics(biometrics),
                name,
                email,
                password: hashedPassword,
            });
            return this.athleteRepository.save(athlete);
        }
        catch (e) {
            this.handleRegisterErrors(e);
        }
    }
    async hash(password) {
        const salt = await Bcrypt.genSalt();
        return Bcrypt.hash(password, salt);
    }
    handleRegisterErrors(e) {
        throw e.message === repository_errors_enum_1.RepositoryErrors.DUPLICATED_ENTRY
            ? new common_1.ConflictException('Username already taken')
            : e;
    }
    async sendConfirmationEmail(athleteId) {
        const athlete = await this.athleteRepository.findById(athleteId);
        await this.emailGateway.sendConfirmationEmail(athlete);
        return athlete;
    }
};
AuthService = __decorate([
    (0, common_1.Injectable)(),
    __param(1, (0, common_1.Inject)(athlete_repository_interface_1.ATHLETE_REPOSITORY)),
    __param(2, (0, common_1.Inject)(email_gateway_1.EmailGatewayToken)),
    __metadata("design:paramtypes", [jwt_1.JwtService, Object, Object])
], AuthService);
exports.AuthService = AuthService;
//# sourceMappingURL=auth.service.js.map